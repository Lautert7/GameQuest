...
public class FrmReajustarpreco extends javax.swing.JFrame {
    ...
    public FrmReajustarpreco(EstoqueService estoqueService) {
    ...
    private void carregarProdutos() {
        try {
            EstoqueService service = estoqueService != null ? estoqueService : clienteRMI.getService();
    ...
    private void JBAumentarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_JBAumentarActionPerformed
        ...
        try {
            String percentualTexto = JTFPercentual.getText().trim();
            if (percentualTexto.isEmpty()) {
                JOptionPane.showMessageDialog(this, "Informe um percentual de reajuste.");
                return;
            }
            ...
            for (Produto p : produtos) {
                double novoPreco = p.getPreco() * (1 + (percentual / 100));
                p.setPreco(novoPreco);
                estoqueService.atualizarProduto(p);
            }
            
            JOptionPane.showMessageDialog(this, "Reajuste de " + percentual + "% aplicado com sucesso!");
            atualizarPrecoAtual();
        } catch (NumberFormatException e) {
            JOptionPane.showMessageDialog(this, "Percentual inválido. Digite um número válido.");
        } catch (Exception e) {
            JOptionPane.showMessageDialog(this, "Erro ao aplicar reajuste: " + e.getMessage());
        }
